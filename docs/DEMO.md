...
## Fastest path (no API key required)
...
Any typed question now runs through live generation. The RAG path (medical/current-affairs-domain
queries) needs `chromadb` installed, which `pip install -r requirements.txt` handles - if it's
somehow missing, that route returns a clear `503` explaining why, rather than silently falling
