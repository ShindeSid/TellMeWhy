...
export function RoutingSummary() {
  ...
  if (!decision) {
    return <p className="text-sm text-neutral-500">Figuring out how best to answer this...</p>;
  }
    ...
    <div className="flex flex-col gap-2 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-0.5 text-sm text-neutral-500">{ROUTE_WHY[decision.route] ?? decision.rationale}</p>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-neutral-400">{why}</span>
      </div>

      <details className="text-xs text-neutral-500">
        <summary className="cursor-pointer select-none text-neutral-400 hover:text-neutral-600">
          Technical details
        </summary>
        <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1">
          ...
          <dt>Reasoning</dt>
          <dd className="col-span-2 text-neutral-400">{decision.rationale}</dd>
        </dl>
      </details>
    </div>
  );
